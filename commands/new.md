...
3. **Fill in the record** (this is the valuable part — do not leave template placeholders):
   - **Style**: if `memory/STYLE.md` exists, write `rule:` and body per it — caveman-terse, unambiguous, exact paths/symbols.
   ...
   - `assertions:` make it machine-verifiable wherever possible:
     ...
     - `forbidden` for anti-patterns (glob + regex, with an `allow:` list for sanctioned exceptions; globs are git pathspecs — use `:(glob)dir/**/*.ext`, and confirm coverage with `git ls-files -- '<glob>'`)
