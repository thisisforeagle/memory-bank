...
- If it **fails**: do NOT force it. Reconcile the failures first (see /memory-bank:check semantics: bump record values for legitimate code changes, fix code for violations), then re-run sync.
...
Use this after: adding/updating records, fixing drift failures, or at the end of a `/memory-bank:review`.
